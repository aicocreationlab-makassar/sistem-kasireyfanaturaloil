...
export async function getTodaySummary() {
  ...
  const [{ data: sales, error }, { data: products }] = await Promise.all([
    supabase.from("sales").select("id,total_amount,total_cogs,gross_profit").eq("status", "completed").gte("created_at", start).lte("created_at", end),
    supabase.from("products").select("stock_quantity").eq("is_active", true),
  ]);
  if (error) throw error;
  const rows = sales ?? [];
  return {
    revenue: rows.reduce((sum, sale) => sum + Number(sale.total_amount), 0),
    cogs: rows.every((sale) => sale.total_cogs !== null) ? rows.reduce((sum, sale) => sum + Number(sale.total_cogs), 0) : null,
    profit: rows.every((sale) => sale.gross_profit !== null) ? rows.reduce((sum, sale) => sum + Number(sale.gross_profit), 0) : null,
...
export async function getRecentSales(limit = 20) {
  const { supabase } = await getSessionContext();
  const { data, error } = await supabase.from("sales").select("*,profiles!sales_cashier_id_fkey(full_name)").order("created_at", { ascending: false }).limit(limit);
  if (error) throw error;
...
export async function getSale(id: string) {
  const { supabase } = await getSessionContext();
  const [{ data: sale, error }, { data: items }] = await Promise.all([
  ...
  if (error) throw error;
  return { sale: sale as unknown as Sale, items: (items ?? []) as SaleItem[] };
}

export async function getMovements(limit = 50) {
  const { supabase } = await getSessionContext();
  const { data, error } = await supabase.from("stock_movements").select("*,products(name,size_ml),profiles!stock_movements_created_by_fkey(full_name)").order("created_at", { ascending: false }).limit(limit);
  if (error) throw error;
...
export async function getDashboardData() {
  ...
  const [{ data: sales, error }, { data: items }, products, recent] = await Promise.all([
    supabase.from("sales").select("id,created_at,total_amount").eq("status", "completed").gte("created_at", range.start).lte("created_at", range.end),
    supabase.from("sale_items").select("product_id,product_name_snapshot,quantity,sales!inner(status,created_at)").eq("sales.status", "completed").gte("sales.created_at", range.start).lte("sales.created_at", range.end),
    getProducts(), getRecentSales(5),
  ]);
  if (error) throw error;
  ...
  return { trend, best, lowStock: products.filter((p) => p.stock_quantity <= p.low_stock_threshold).sort((a,b)=>a.stock_quantity-b.stock_quantity), recent };
}
